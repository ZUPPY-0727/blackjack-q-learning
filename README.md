...
本プロジェクトは、Anaconda 上に作成した Python 仮想環境で実行した。

- OS: Windows
- Python: 3.13
- 仮想環境: Anaconda
- パッケージ管理: pip
- 実行ターミナル: Anaconda Powershell Prompt
- バージョン管理: Git / GitHub

## 環境構築

### 1. Anaconda をインストール
Anaconda をインストールし、Windows では **Anaconda Powershell Prompt** を起動して作業を行う。

### 2. 仮想環境を作成
以下のコマンドで仮想環境を作成する。

```bash
conda create -n envA python=3.13
```
...
- `dealer.py`が接続待ち状態になったら、もう一方のターミナルでプレイヤープログラムを実行します。
- プレイヤープログラムが終了しても`dealer.py`は終了せず接続待ち状態に戻るため、続けて別のプレイヤープログラムを実行できます。
...
ディーラー・プレイヤー間の通信に失敗する場合は、接続先を`127.0.0.1`にそろえると改善することがあります。
- `dealer.py`
```python
...
- εと学習率をゲーム進行に応じて変化
- 学習後半の平均報酬を`avg_reward_log.csv`に保存
- 平均報酬が過去最高になった場合、Qテーブルを`q_best.pkl`に保存
...
```bash
# 一から学習する場合
python src/ai_player_Q.py --games 100000 --history play_log.csv --save QTable.pkl

# 学習済みQテーブルをロードして学習を再開する場合
python src/ai_player_Q.py --games 50000 --history play_log.csv --load QTable.pkl --save new_QTable.pkl

# 学習済みQテーブルをロードしてテストプレイのみ行う場合
python src/ai_player_Q.py --games 1000 --history test_log.csv --load q_best.pkl --testmode
```
...
オリジナル版と最終改良版について、テストモード10000回の実行で得られた結果を比較した。  
比較結果は `results` フォルダにまとめている。


### 比較グラフ
- `results/original_vs_improved.png`
  - オリジナル版と最終改良版の累積収支の推移を比較したグラフ

![オリジナル版と最終改良版の累積収支比較](results/original_vs_improved.png)

### 比較表
- `results/original_vs_improved_summary.csv`
  - オリジナル版と最終改良版の比較結果をまとめたCSVファイル

### 補足
- 詳細な改善過程や、どのような変更を加えたかについては Qiita 記事にまとめる予定
...
改善の過程や考察については、以下の記事にまとめています。  
[Qiita記事はこちら](https://qiita.com/ZUPPY-0727/private/ca6b95a9f8383392d801)
